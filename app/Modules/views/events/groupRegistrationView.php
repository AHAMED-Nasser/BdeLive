...
<div class="container" style="max-width: 700px; margin: 40px auto; padding: 20px;">
    ...
    <?php foreach (['success' => '#d4edda', 'error' => '#f8d7da', 'warning' => '#fff3cd'] as $type => $color) : ?>
        <?php if (!empty($flash[$type])) : ?>
            <div style="background-color: <?= $color ?>; color: #<?= $type === 'success' ? '155724' : ($type === 'error' ? '721c24' : '856404') ?>; padding: 12px; margin: 20px 0; border-radius: 4px; text-align: center;">
                <?= htmlspecialchars($flash[$type]) ?>
            </div>
        <?php endif; ?>
    <?php endforeach; ?>

    <div class="group-info" style="background: #e3f2fd; padding: 20px; border-radius: 8px; margin-bottom: 30px; border-left: 4px solid #2196f3;">
    ...
    <div class="team-size-info" style="text-align: center; margin-bottom: 30px; padding: 15px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 8px; color: white;">
    ...
    <form action="index.php?page=groupRegistration&event_id=<?= $event['event_id'] ?>" method="POST" class="group-form">
        ...
        <div class="creator-info" style="background: #e8f5e9; padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #4caf50;">
        ...
        <div class="warning-box" style="background: #fff3cd; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #ffc107;">
        ...
        <div class="form-actions" style="margin-top: 30px;">
            <button type="submit" style="width: 100%; padding: 15px; background: linear-gradient(135deg, #28a745 0%, #20c997 100%); color: white; border: none; border-radius: 8px; font-size: 16px; font-weight: bold; cursor: pointer; transition: transform 0.2s;">
    ...
    <?php if (!empty($userTeams)) : ?>
        <div class="existing-teams" style="margin-top: 40px; padding-top: 30px; border-top: 2px solid #eee;">
            ...
            <?php foreach ($userTeams as $team) : ?>
                <div class="team-card" style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin-bottom: 10px; border-left: 4px solid <?= $team['status'] === 'confirmed' ? '#28a745' : ($team['status'] === 'pending' ? '#ffc107' : '#dc3545') ?>;">
                    <strong>Groupe <?= $team['team_number'] ?></strong>
                    <span style="float: right; font-size: 12px; padding: 4px 8px; border-radius: 4px; background: <?= $team['status'] === 'confirmed' ? '#d4edda' : ($team['status'] === 'pending' ? '#fff3cd' : '#f8d7da') ?>; color: <?= $team['status'] === 'confirmed' ? '#155724' : ($team['status'] === 'pending' ? '#856404' : '#721c24') ?>;">
