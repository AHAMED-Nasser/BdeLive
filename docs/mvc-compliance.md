...
|----------|------|----------------|
| **Model**  | Données + logique métier + persistance | **Entities** (Article, Event), **Repositories** (accès BDD), **Factories** (PDO → Entity). Le contrôleur ne fait pas de SQL, il utilise les repositories. |
...
Requête HTTP
    ...
    → Controller (ex. ShowEventController) reçoit EventRepository
...
- **Model = données + persistance** : Entities (logique métier) + Repositories (persistance) + Factories (hydratation). Les contrôleurs dépendent des **repositories** (EventRepository, ArticleRepository) injectés par le Container.
...
- **Injection de dépendances** : les contrôleurs reçoivent leurs dépendances (repositories) par le constructeur via le Container ; pas de `new EventRepository()` dans les contrôleurs qui sont résolus par le Container.
